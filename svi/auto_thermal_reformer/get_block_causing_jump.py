...
# This code exemplifies how to extract the blocks that are causing this jump for this
# specific instance.
...
def calculate_condition_number(m, condition_number_threshold = 1e6):
...
def id_blocks_high_condition_number(dict1, dict2):
    increased_blocks = []
...
def main():
    for i, (full_subset) in enumerate(zip(SUBSETS_FULLSPACE), 1):
        before_jump = full_space(X=full_subset[0][0], P=full_subset[0][1], iters=full_subset[0][2])
        after_jump = full_space(X=full_subset[0][0], P=full_subset[0][1], iters=full_subset[0][2]+1)
    block_to_analyze = id_blocks_high_condition_number(before_jump, after_jump)
    # Maybe also print equations and variables present in that block? For now, all we need is the number.
    # With this number we will proceed to identify and print variable values participating in this block.
