...
def make_simulation_model(P,initialize=True):
    ...
    m.fs.reformer_mix.steam_inlet.flow_mol.fix(245)
    solver = get_solver()
...
def make_optimization_model(X,P,initialize=True):
    ...
    """
    m = make_simulation_model(P,initialize=initialize)
...
df = {'X':[], 'P':[], 'Termination':[], 'Time':[], 'Objective':[], 'Steam':[], 'Bypass Frac': []}

def main(X,P):
    m = make_optimization_model(X,P)
    add_external_function_libraries_to_environment(m)
    m_implicit = make_implicit(m)
    solver = pyo.SolverFactory("cyipopt", options = {"tol": 1e-6, "max_iter": 1000})
    timer = TicTocTimer()
...
if __name__ == "__main__":
    for X in np.arange(0.85,0.90,0.007):
        for P in np.arange(847379,1347379,70000):
            try:
                main(X,P)
...
df = pd.DataFrame(df)
df.to_csv('imp_param_sweep_test2.csv')