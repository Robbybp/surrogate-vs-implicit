...
def set_atr_flowsheet_inputs(T,P,m):
...
def make_simulation_model(T, P, F, BP, initialize=True):
    m = pyo.ConcreteModel(name="ATR_Flowsheet")
    m.fs = FlowsheetBlock(dynamic=False)
    build_atr_flowsheet(m)
    set_atr_flowsheet_inputs(T,P,m)
    if initialize:
        initialize_atr_flowsheet(m)

    #
    # Fix degrees of freedom for simulation model
    # (Fix steam flow and bypass fraction, add one conversion constraint)
    #
    m.fs.reformer_bypass.split_fraction[0, "bypass_outlet"].fix(BP)
    m.fs.reformer_mix.steam_inlet.flow_mol.fix(F)
...
basename = "alamo_param_sweep_conv94.csv"
fname = os.path.join(dirname,basename)
df = pd.read_csv(fname)
df = df[['T','P','Steam','Bypass Frac']]
...
if __name__ == "__main__":
    ...
    if simulation:  
        for index, row in df.iterrows():
            ...
            F = row['Steam']
            BP = row['Bypass Frac']
            m = make_simulation_model(T, P, F, BP, initialize=True)
            ...
            df2[list(df2.keys())[0]].append(T)
            df2[list(df2.keys())[1]].append(P)
            df2[list(df2.keys())[2]].append(value(m.fs.product.mole_frac_comp[0, 'H2']))
    
    df2 = pd.DataFrame(df2)
    df2.to_csv('validate_alamo_with_square_conv94.csv')
