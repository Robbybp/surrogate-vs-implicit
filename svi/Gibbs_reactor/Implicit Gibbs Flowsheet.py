...
m_implicit.objective = pyo.Reference(m.fs.objective)
...
m_implicit.conv_constraint = Constraint(
    ...
    * m_implicit.fs.H101.outlet.mole_frac_comp[0, "CH4"]
    == (
        ...
        - m_implicit.input_vars[4].value * m_implicit.input_vars[5].value
    )
)
...
m_implicit.link_R101_imp = Constraint(expr = m.fs.R101.inlet.temperature[0] == m_implicit.fs.H101.outlet.temperature[0])
m_implicit.link_R101_imp = Constraint(expr = m.fs.R101.inlet.pressure[0] == m_implicit.fs.H101.outlet.pressure[0])
m_implicit.link_R101_imp = Constraint(expr = m.fs.R101.inlet.flow_mol[0] == m_implicit.fs.H101.outlet.flow_mol[0])
...
## Error Message:
# ---> 93 solver.solve(m_implicit, tee=True)

# File c:\Users\sbugo\anaconda3\envs\my-practice-env\lib\site-packages\pyomo\contrib\pynumero\algorithms\solvers\cyipopt_solver.py:548, in PyomoCyIpoptSolver.solve(self, model, **kwds)
#     544 grey_box_blocks = list(model.component_data_objects(
#     545     egb.ExternalGreyBoxBlock, active=True))
#     546 if grey_box_blocks:
#     547     # nlp = pyomo_nlp.PyomoGreyBoxNLP(model)
# --> 548     nlp = pyomo_grey_box.PyomoNLPWithGreyBoxBlocks(model)
#     549 else:
#     550     nlp = pyomo_nlp.PyomoNLP(model)

# File c:\Users\sbugo\anaconda3\envs\my-practice-env\lib\site-packages\pyomo\contrib\pynumero\interfaces\pyomo_grey_box_nlp.py:99, in PyomoNLPWithGreyBoxBlocks.__init__(self, pyomo_model)
#      97             fixed_vars.extend(v for v in data.inputs.values() if v.fixed)
#      98             fixed_vars.extend(v for v in data.outputs.values() if v.fixed)
# ---> 99             greybox_nlp = _ExternalGreyBoxAsNLP(data)
#     100             greybox_nlps.append(greybox_nlp)
#     102 if fixed_vars:

# File c:\Users\sbugo\anaconda3\envs\my-practice-env\lib\site-packages\pyomo\contrib\pynumero\interfaces\pyomo_grey_box_nlp.py:524, in _ExternalGreyBoxAsNLP.__init__(self, external_grey_box_block)
#     522 self._primal_values = np.copy(self._init_primals)
# ...
